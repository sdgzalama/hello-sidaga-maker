	...
	function boldthemes_get_media_html( $type, $data ) {
		...
		if ( $type == 'image' ) {
			...
			$html = '<div class="btMediaBox">' . do_shortcode( '[bt' . '_grid_gallery ids="' . join( ',', $data[0] ) . '" columns="' . esc_attr( $data[1] ) . '" has_thumb="' . 
				esc_attr( $data[2] ) . '" format="' . esc_attr( $data[3] ) . '" lightbox="' . esc_attr( $data[4] ) . '" grid_gap="' . esc_attr( $data[5] ) . '" el_class="' . esc_attr( $data[6] ) . '"]' ) . '</div>';
 ...
 */
 if ( ! function_exists( 'boldthemes_get_icon_html' ) ) {
	function boldthemes_get_icon_html( $icon, $url, $text, $el_class, $target = "", $el_style = "", $url_text = "" ) { 
		...
		return '<span class="btIco ' . esc_attr( $el_class ) . '" ' . wp_kses_post( $style_attr ) . '><' . wp_kses_post( $ico_tag ) . ' data-ico-' . esc_attr( $icon_set ) . '="&#x' . esc_attr( $icon ) . ';" class="btIcoHolder"><em>' . esc_html( $url_text ) . '</em>' . wp_kses_post( $text ) . '</' . wp_kses_post( $ico_tag_end ) . '></span>';
	}
}
 ...
 */
 ...
 if ( ! function_exists( 'boldthemes_get_image_html' ) ) {
	function boldthemes_get_image_html( $image, $lazy_load, $caption, $caption_text, $size, $shape, $url, $target, $show_titles, $el_style, $el_class, $alt = '' ) {
		...
		if ( $lazy_load ) {
			$output = '<div class = "btImage"><img src="' . esc_url_raw( get_template_directory_uri() . '/gfx/blank.gif' ) . '" data-image_src="' . ( $image ) . '" alt="' . esc_attr( $alt ) . '" class="btLazyLoadImage" ></div>';
		} else {
			$output = '<div class = "btImage"><img src="' . esc_url_raw( $image ) . '" alt="' . esc_attr( $alt ) . '" title="' . esc_attr( $alt ) . '" ></div>';	
		}
		...
		if ( strpos( $url, '<a href' ) === 0 ) {
			...
			$link = '<a href="' . esc_url_raw( $link ) . '" target="' . esc_attr( $target ) . '" title="' . esc_attr( $alt ) . '"></a>';
		}
