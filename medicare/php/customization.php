...
if ( ! function_exists( 'boldthemes_logo' ) ) {
	function boldthemes_logo( $type = 'header' ) {
		...
		if ( $logo != '' ) {
			$logo_hw_transient_key = 'bt_logo_hw_' . $logo;
			$logo_hw_transient_value = get_transient( $logo_hw_transient_key );
			if ( false === $logo_hw_transient_value ) {
				// Transient expired or not set
				$hw = '';
				$logo_size = @getimagesize( $logo );
				if ( is_array( $logo_size ) && $logo_size[1] > 0 ) {
					$hw = $logo_size[0] / $logo_size[1];
				}
				set_transient( $logo_hw_transient_key, $hw, 2*60*60 ); // transient expires in 2 hours
			} else {
				// Use the transient value
				$hw = $logo_hw_transient_value;
			}
		}
